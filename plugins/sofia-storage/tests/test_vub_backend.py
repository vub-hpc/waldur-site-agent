"""Tests for VUB GPFS Storage backend."""
...
from gpfs.backend import VubStorageBackend

from waldur_site_agent.backend.exceptions import BackendError


class TestVubStorageBackend:
    """Test cases for VUB GPFS Storage backend."""

    def setup_method(self):
        """Set up test environment."""
        self.backend_settings = {
            "storage_file_system": "gpfs",
        ...
        self.backend = VubStorageBackend(self.backend_settings, self.backend_components)

    def test_backend_initialization(self):
        """Test backend initialization."""
        assert self.backend.backend_type == "vub-storage-plugin"
        assert self.backend.storage_file_system == "gpfs"
    ...
    def test_generate_mount_point(self):
        ...
        mount_point = self.backend._generate_mount_point(
            storage_system="gpfs-fs",
            tenant_id="university",
            customer="physics-dept",
            project_id="climate-sim",
        )
        assert mount_point == "/gpfs-fs/store/university/physics-dept/climate-sim"
    ...
    def test_create_storage_resource_json(self):
        ...
        storage_json = self.backend._create_storage_resource_json(mock_resource, "gpfs-fs", None)

        assert storage_json["itemId"] == mock_resource.uuid.hex
        assert storage_json["status"] == "pending"
        assert (
            storage_json["mountPoint"]["default"] == "/gpfs-fs/store/cscs/university/physics-dept"
        )
        assert storage_json["permission"]["value"] == "2770"
        assert len(storage_json["quotas"]) == 4  # 2 space + 2 inode quotas
        assert storage_json["storageSystem"]["key"] == "gpfs-fs"
        assert storage_json["storageFileSystem"]["key"] == "gpfs"
    ...
    def test_create_storage_resource_json_with_provider_action_urls(self):
        ...
        storage_json = self.backend._create_storage_resource_json(
            mock_resource, "gpfs-fs", mock_client
        )
    ...
    @patch("gpfs.backend.marketplace_resources_list")
    ...
    def test_unset_offering_slug_validation(self):
        ...
        backend = VubStorageBackend(self.backend_settings, self.backend_components)
    ...
    def test_pagination_header_case_insensitive(self):
        ...
        with patch(
            "gpfs.backend.marketplace_resources_list"
        ) as mock_list:
            mock_response = Mock()
    ...
    def test_system_identifiers_use_deterministic_uuids(self):
        ...
        assert storage_file_system["key"] == "gpfs"
    ...
    def test_get_project_unix_gid_with_hpc_user_client(self):
        ...
        backend = VubStorageBackend(backend_settings, self.backend_components)
    ...
    @patch("gpfs.backend.marketplace_resources_list")
    @patch(
        "gpfs.backend.marketplace_provider_offerings_customers_list"
    )
