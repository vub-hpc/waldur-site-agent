...
class TestSyncScript:
    """Test cases for VUB GPFS Storage sync script."""
    ...
    def setup_method(self):
        ...
        self.offering_config = {
            ...
            "backend_type": "vub-storage-plugin",
            "backend_settings": {
    ...
    @patch("gpfs.sync_script.get_client")
    @patch("gpfs.sync_script.VubStorageBackend")
    ...
    @patch("gpfs.sync_script.load_configuration")
    @patch("gpfs.sync_script.sync_offering_resources")
    @patch("sys.argv", ["waldur_vub_storage_sync", "--config", "/fake/config.yaml"])
