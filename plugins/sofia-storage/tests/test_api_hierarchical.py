...
pytestmark = pytest.mark.skipif(
    not os.getenv("WALDUR_VUB_STORAGE_PROXY_CONFIG_PATH"),
    reason="WALDUR_VUB_STORAGE_PROXY_CONFIG_PATH not set - skipping API integration tests",
)

try:
    from gpfs.waldur_storage_proxy.main import app
except SystemExit:
...
class TestHierarchicalStorageAPI:
    ...
    @patch(
        "gpfs.backend.VubStorageBackend._get_offering_customers"
    )
    @patch(
        "gpfs.backend.VubStorageBackend._get_resources_by_offering_slugs"
    )
    ...
    @patch(
        "gpfs.backend.VubStorageBackend._get_resources_by_offering_slug"
    )
